...
def get_next_user_message(session_num: int, msg_num: int) -> str:
    return f"Tell me more about that. (User message #{msg_num})"
            ...
            if 'message' in chunk:
                ...
                if LOG_THINKING:
                    ...
                    for part in thinking_parts:
                        if part:
                            print(f"\033[90m{part}\033[0m", end='', flush=True)
                            full_thinking += part
...
def main():
    ...
    for session_index in range(MAX_SESSION_NUM):
        ...
        while running and session_mgr.is_run_exists():
            ...
            if len(history) >= 10:
                log.info(f"Session {session_num}: context limit reached (simulated), completing session.")
                session_mgr.complete_current_session()
                break
